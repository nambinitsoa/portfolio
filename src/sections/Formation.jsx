...
import { FaGraduationCap } from "react-icons/fa"; // Icône de graduation
          ...
          {formations.map((item) => (
            <div key={item.id} className="formation-item">
              ...
              <div className="formation-card">
                {/* Ligne avec icône + école */}
                <div className="flex items-center gap-3 mb-2">
                  <FaGraduationCap className="text-cyan-500 text-xl" />
                  <h3 className="formation-school">{item.school}</h3>
                </div>
                <h4 className="formation-diploma">{item.diploma}</h4>
                <span className="formation-date">{item.date}</span>
